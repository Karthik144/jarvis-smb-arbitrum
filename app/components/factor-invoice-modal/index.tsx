...
export default function FactorInvoiceModal({
  ...
  function stepLabel(): string {
    switch (step) {
      case "factoring":
        return "Step 1/2: Factoring invoice on Robinhood…";
      case "redirecting":
        return "Step 2/2: Redirecting escrow payment to lender on Arbitrum…";
      case "done":
        return "Invoice factored! Payment redirected to lender.";
      case "error":
        return `Error: ${error}`;
      default:
        return "";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const robinhoodSigner = await getSignerOnChain(
        wallet,
        ROBINHOOD_CHAIN_ID
      );
      ...
      setStep("redirecting");
      const arbitrumSigner = await getSignerOnChain(
        wallet,
      ...
      // --- Step 3: Update database with factored invoice ---
      ...
      if (!data.success) {
        console.error("Failed to save factored invoice to DB:", data.error);
      }
    ...
    >
      <DialogContent sx={{ p: 4 }}>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          >
            Get immediate payment for your remaining invoice amount at a
            discount.
          </Typography>
          ...
          >
            ...
            <Typography
              sx={{ fontSize: "20px", fontWeight: 600, fontFamily: "inherit" }}
            >
            ...
            >
              Upfront paid: ${upfrontPaid.toFixed(2)} • Remaining: $
        ...
        <Box sx={{ display: "flex", gap: 2, mb: 3 }}>
          <Box
            ...
            sx={{
              flex: 1,
              border:
                selectedRate === 5 ? "2px solid #171717" : "1px solid #E0E0E0",
              borderRadius: "10px",
              p: 2,
              cursor: "pointer",
          ...
          <Box
            ...
            sx={{
              flex: 1,
              border:
                selectedRate === 10 ? "2px solid #171717" : "1px solid #E0E0E0",
              borderRadius: "10px",
              p: 2,
              cursor: "pointer",
          ...
          >
            ...
            >
              You get: ${(remainingAmount * 0.9).toFixed(2)}
            </Typography>
          </Box>
        </Box>
        ...
        <Box sx={{ display: "flex", gap: 2 }}>
          ...
          <Button
            ...
            disabled={
              step === "factoring" || step === "redirecting" || step === "done"
            }
            fullWidth
            sx={{
